...
// Return false no es correcto, true es correcta
    ...
    /**
     * Función para validar una hora en formato HH:MM.
     * 
     * @author Oscar Pascual Ferrero.
     * @version 1.0 Creada la función.
     * @since 2023-11-09
     * @param string $hora Hora que se va a comprobar.
     * @param boolean $obligatorio Valor booleano indicado mediante 1 si es obligatorio o 0 si no lo es.
     * @return null|string Devuelve null si es correcto o un mensaje de error en caso de que lo haya.
     */
    public static function validarHora($hora, $obligatorio = 0) {
        $mensajeError = null;
        ...
        if (!preg_match($patron, $hora) && !empty($hora)) {
            $mensajeError .= " La hora debe estar en formato HH:MM y ser válida.";
        }
    ...
    /**
     * Función para validar un color en formato hexadecimal (#RRGGBB o #RGB).
     * 
     * @author Oscar Pascual Ferrero.
     * @version 1.0 Creada la función.
     * @since 2023-11-09
     ...
     * @return null|string Devuelve null si es correcto o un mensaje de error en caso de que lo haya.
     */
    public static function validarColorHexadecimal($color, $obligatorio = 0) {
        $mensajeError = null;

        // Eliminar el signo "#" si está presente
        $color = ltrim($color, '#');

        // Comprobar si el color es una cadena hexadecimal válida
        if (!preg_match('/^([A-Fa-f0-9]{3}){1,2}$/', $color) && !empty($color)) {
            $mensajeError = "El color no es válido. Debe ser un formato hexadecimal (#RRGGBB o #RGB).";
        }
